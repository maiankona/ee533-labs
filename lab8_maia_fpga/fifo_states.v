...
module convertible_fifo (
    ...
    // Port A: write address and data (writes only - no read-before-write needed)
    assign bram_addr_a = (state == PROCESSING)   ? bram_addr :
                         (state == TRANSMITTING) ? tx_ptr : tail_reg;

    // Data MUX: pipeline store vs host write vs network. GPU/host: use {8'h0, data} (no preserve)
    assign bram_din_a  = (state == PROCESSING) ? {8'h0, (pipeline_we ? pipeline_data : data_in_64)} : net_rx_data;

    // Port B: read address - used for host reads and pipeline loads (dedicated read port)
    assign bram_addr_b = (state == PROCESSING) ? bram_addr : 8'h0;

    // Write Enable: BRAM write when (host write to data) OR (pipeline store)
    wire bram_we_cpu = fifo_data && cpu_we;
    wire bram_we_pipeline = pipeline_we;
    assign bram_we = (state == PROCESSING && (bram_we_cpu || bram_we_pipeline)) ? 1'b1 :
    ...
    // Dual-port BRAM: Port A = write, Port B = read (like user's working convertible_fifo)
    ...
    // Read data from Port B (host reads + pipeline loads)
    assign cpu_data_out = thread_has_access
        ? (fifo_head ? {24'b0, head_reg} : (fifo_tail ? {24'b0, tail_reg} : (fifo_data ? bram_dout_b[31:0] : 32'b0)))
        : 32'b0;
    assign raw_data_out = bram_dout_b;
