...
module pipeline_backup(
    ...
    // DMEM interface: host (like pipeline_pseudoARM + memory) - FIFO inside pipeline
    input         write_to_dmem,
    input         read_req_dmem,
    input  [7:0]  addr_dmem_host,
    input  [31:0] data_dmem_host,
    input         gpu_mode,       // 1 = FIFO in PROCESSING (imem_loaded)
    output [63:0] data_out_dmem, // For host read (ids captures lo/hi based on addr)
    ...
    // Also stall when tensor unit cannot accept a new command.
    assign stall = stall_hzd | (id_ex_is_tensor & ~tensor_cmd_ready);
    ...
    gpu_fifo_mem ME (
        ...
        .pipeline_mem_read (pipeline_mem_read),
        .gpu_mode       (gpu_mode),
        .dmem_out       (dmem_out)
    );
    assign data_out_dmem = dmem_out;
