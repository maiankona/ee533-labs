...
module tb_cpu_fifo_load_store;
    ...
    task write_arm_dmem;
        ...
        input [31:0] data;
        begin
            force dut.arm_cpu.cpu_mem_addr = {24'h0, addr};
            force dut.arm_cpu.cpu_mem_data = data;
            force dut.arm_cpu.cpu_mem_we   = 1'b1;
            @(posedge clk); #1;
            force dut.arm_cpu.cpu_mem_we   = 1'b0;
        end
    endtask
    ...
    task read_arm_dmem;
        ...
        begin
            force dut.arm_cpu.cpu_mem_addr = {24'h0, addr};
            @(posedge clk); #1;
            data = dut.arm_cpu.cpu_mem_data;
            release dut.arm_cpu.cpu_mem_addr;
        end
    endtask
    ...
    initial begin
        ...
        $display("  Loading Load / Store Program...");
            write_arm_imem(9'h000, 32'hE3A03003); // MOV R3, #3
			write_arm_imem(9'h001, 32'hE1A00000); // NOP  (wait for MOV to WB)
			write_arm_imem(9'h002, 32'hE1A00000); // NOP
			write_arm_imem(9'h003, 32'hE1A00000); // NOP
			write_arm_imem(9'h004, 32'hE5934000); // LDR R4, [R3]  -> R4 = DMEM[3] = 0xCAFEBABE
			write_arm_imem(9'h005, 32'hE1A00000); // NOP
			write_arm_imem(9'h006, 32'hE1A00000); // NOP
			write_arm_imem(9'h007, 32'hE5834000); // STR R4, [R3]  -> DMEM[3] = R4
