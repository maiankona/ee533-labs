...
module tb_ann_neuron;
    ...
    initial begin
        ...
        // 5 NOPs: pipeline depth (4) + BRAM register file read latency (1)
        write_arm_imem(9'h005, ARM_NOP);          // NOP - pipeline fill 1
        write_arm_imem(9'h006, ARM_NOP);          // NOP - pipeline fill 2
        write_arm_imem(9'h007, ARM_NOP);          // NOP - pipeline fill 3
        write_arm_imem(9'h008, ARM_NOP);          // NOP - pipeline fill 4
        write_arm_imem(9'h009, ARM_NOP);          // NOP - pipeline fill 5
        write_arm_imem(9'h00A, ARM_STR_R1_R3);   // STR R1,[R3] -> 0xF0  R3 valid
        write_arm_imem(9'h00B, ARM_NOP);          // NOP - pipeline fill 1
        write_arm_imem(9'h00C, ARM_NOP);          // NOP - pipeline fill 2
        write_arm_imem(9'h00D, ARM_NOP);          // NOP - pipeline fill 3
        write_arm_imem(9'h00E, ARM_NOP);          // NOP - pipeline fill 4
        write_arm_imem(9'h00F, ARM_NOP);          // NOP - pipeline fill 5
        write_arm_imem(9'h010, ARM_STR_R2_R4);   // STR R2,[R4] -> 0xF1  R4 valid
        write_arm_imem(9'h011, ARM_NOP);          // NOP - pipeline fill 1
        write_arm_imem(9'h012, ARM_NOP);          // NOP - pipeline fill 2
        write_arm_imem(9'h013, ARM_NOP);          // NOP - pipeline fill 3
        write_arm_imem(9'h014, ARM_NOP);          // NOP - pipeline fill 4
        write_arm_imem(9'h015, ARM_NOP);          // NOP - pipeline fill 5
        write_arm_imem(9'h016, ARM_STR_R1_R5);   // STR R1,[R5] -> 0xF2  R5 valid - dispatch!
        write_arm_imem(9'h017, ARM_B_SELF);       // B . (halt)
