...
module pipeline_pseudoARM(
    ...
    // CPU start & enables
    reg cpu_start_latch, cpu_en, cpu_en_1, cpu_en_2;
    ...
    // =========================================================
    // BRIDGE 2: ID/EX
    // id_r1data and id_r2data now carried through bridge so EX
    // uses operands belonging to the instruction in EX, not ID.
    // Width: 9(br_addr)+1(branch)+4(cond)+32(r1)+32(r2)+26(ctrl) = 104 bits
    // =========================================================
    wire [103:0] id_ex_din = {
        id_br_addr,     // [103:95]  9 bits
        id_branch,      // [94]      1 bit
        id_cond,        // [93:90]   4 bits
        id_r1data,      // [89:58]   32 bits
        id_r2data,      // [57:26]   32 bits
        id_curr_thread, // [25:24]   2 bits
        id_alu_ctrl,    // [23:20]   4 bits
        id_immediate,   // [19:8]    12 bits
        id_alu_src,     // [7]       1 bit
        id_mem_to_reg,  // [6]       1 bit
        id_wmem_en,     // [5]       1 bit
    ...
    wire [31:0] ex_r1data      = id_ex_q[89:58];   // correctly pipelined R1
    wire [31:0] ex_r2data      = id_ex_q[57:26];   // correctly pipelined R2
    ...
    assign ex_br_addr       = ex_br_addr_q;
    assign ex_branch_thread = ex_curr_thread;
    assign ex_br_target     = ex_br_addr_q;

    // --- STAGE 3: EXECUTE (EX) ---
    // Both operands now correctly pipelined
    ...
    wire [72:0] ex_me_d = {
        ...
        ex_alu_out,      // [70:39]
        ex_r2data,       // [38:7]  correctly pipelined store data
        ex_wmem_en,      // [6]
        ex_mem_to_reg,   // [5]
        ex_waddr,        // [4:1]
        ex_wrege         // [0]
    ...
    // MEM stage outputs for gpu_control_interface_2
    // Only present address/data when write enable is high
    assign cpu_mem_addr = me_wme ? me_alu_res[7:0]       : 8'h00;
    assign cpu_mem_data = me_wme ? {32'h0, me_store_data} : 64'h0;
    ...
    // cpu_en_1/2: two-cycle delay after cpu_start before pipeline unstalls
    // gives BRAM register file time to settle
