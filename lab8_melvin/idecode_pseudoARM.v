...
module idecode (
    ...
    output wire [63:0] alu_b_out,     // select immediate vs register
    //output wire [4:0]  shift_out,      // shift amount / extra ctrl bits REMOVING FOR NOW
    ...
    assign ALUSrc_out    = (is_dp || is_branch && s_bit) ? i_bit : 1'b1; // Memory offsets always use Immediate

    wire [63:0] sign_ext_imm = {{52{id_inst[11]}}, id_inst[11:0]}; // Sign extend 12-bit to 32-bit
    assign alu_b_out = (ALUSrc_out) ? sign_ext_imm : r2data;
    ...
    // REGISTER FILE INSTANCE (The Feedback Loop)
    registerFile64 rf_inst (
        .clk(clk),
        .clr(rst),
        .r1addr({curr_thread, rs1}), 
        .r2addr({curr_thread, rs2_sw}),
        //WB stuff
		  .waddr({wb_thread, wb_waddr}), 
        .wdata(wb_wdata),   
        .wena(wb_wena),   
        //pick up r1 and r2 here
		  .r1data(r1data),
        .r2data(r2data)
    );
