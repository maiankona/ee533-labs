...
// unpack 64-bit data into BF16 lanes
...
wire [15:0] a_lane0 = (is_vadd | is_vsub) ? r1_lane0 :
                      (is_vmul | is_vmac)  ? mul_lane0 : 16'h0000;
wire [15:0] a_lane1 = (is_vadd | is_vsub) ? r1_lane1 :
                      (is_vmul | is_vmac)  ? mul_lane1 : 16'h0000;
wire [15:0] a_lane2 = (is_vadd | is_vsub) ? r1_lane2 :
                      (is_vmul | is_vmac)  ? mul_lane2 : 16'h0000;
wire [15:0] a_lane3 = (is_vadd | is_vsub) ? r1_lane3 :
                      (is_vmul | is_vmac)  ? mul_lane3 : 16'h0000;
// b_lane: accumulator for VMAC, r2 for VADD/VSUB, zero for VMUL and default (NOP/bubble)
wire [15:0] b_lane0 = is_vmac             ? rd_lane0 :
                      (is_vadd | is_vsub)  ? r2_lane0 : 16'h0000;
wire [15:0] b_lane1 = is_vmac             ? rd_lane1 :
                      (is_vadd | is_vsub)  ? r2_lane1 : 16'h0000;
wire [15:0] b_lane2 = is_vmac             ? rd_lane2 :
                      (is_vadd | is_vsub)  ? r2_lane2 : 16'h0000;
wire [15:0] b_lane3 = is_vmac             ? rd_lane3 :
                      (is_vadd | is_vsub)  ? r2_lane3 : 16'h0000;
// Adder / Subtractor
wire [15:0] add_out0, add_out1, add_out2, add_out3;
bf16_add add_lane0 ( .operation(add_sub_op), .a(a_lane0), .b(b_lane0), .result(add_out0));
bf16_add add_lane1 ( .operation(add_sub_op), .a(a_lane1), .b(b_lane1), .result(add_out1));
bf16_add add_lane2 ( .operation(add_sub_op), .a(a_lane2), .b(b_lane2), .result(add_out2));
bf16_add add_lane3 ( .operation(add_sub_op), .a(a_lane3), .b(b_lane3), .result(add_out3));
// RELU
...
// Output mux ? zero out all lanes when no tensor op is active (bubble/NOP)
wire [15:0] out_l0 = ~is_active ? 16'h0000 : is_vrelu ? relu_l0 : add_out0;
wire [15:0] out_l1 = ~is_active ? 16'h0000 : is_vrelu ? relu_l1 : add_out1;
wire [15:0] out_l2 = ~is_active ? 16'h0000 : is_vrelu ? relu_l2 : add_out2;
wire [15:0] out_l3 = ~is_active ? 16'h0000 : is_vrelu ? relu_l3 : add_out3;
// falling edge of is_active ? fires the cycle the result lands in MEM
reg is_active_prev;
always @(posedge clk) is_active_prev <= is_active;
assign tensor_done = ~is_active & is_active_prev;
