...
module ids 
   ...
   reg [31:0]                      packet_check_low;
   reg [31:0]                      packet_check_mid;
   reg [31:0]                      packet_check_high;
   ...
   #( 
      ...
      // --- HW regs interface
	  .hardware_regs    ({packet_check_low, packet_check_mid, packet_check_high, cpu_extract_check, alu_out_check, dmem_out}),

      .clk              (clk),
      .reset            (reset)
    );

   //pass-through so that nothing gets ruined
   ...
   //top level pre-parse
   //wire is_pure_data = (in_ctrl == 8'h00);

   reg in_packet; //state machine in case of consecutive starts
   ...
   pipeline_pseudoARM cpuFIFO (
      ...
      .is_fifo_store(cpu_is_fifo_store),
      .valid_store(cpu_valid_store),
      .fifo_out_intercept(cpu_fifo_out_intercept),
      .packet_content_echo(cpu_packet_content_echo),
      
      .alu_result_detected(alu_result_detected),
      .data_out_dmem(data_out_dmem),
      .alu_out_intercept(alu_out_intercept),
      
      .net_rx_data({in_ctrl, in_data}), //parallel network inputs go here...
      .net_rx_valid(in_wr),
      .net_rx_sop(is_sop),
      .net_rx_eop(is_eop)
   );
   ...
   always @(posedge clk) begin
      if (reset) begin
         ...
         if (dmem_read_req) begin
            dmem_out <= data_out_dmem[31:0]; //the lazy fix, just look over the bottom bits and we're good
         end
         ...
         if (cpu_valid_store && cpu_is_fifo_store) begin
            cpu_extract_check <= cpu_fifo_out_intercept;
            packet_check_high <= {24'b0, cpu_packet_content_echo[71:64]};
            packet_check_mid <= cpu_packet_content_echo[63:32];
            packet_check_low <= cpu_packet_content_echo[31:0];
         end
      end
