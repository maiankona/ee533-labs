...
module ids
   ...
   always @(posedge clk) begin
      if (reset)
         ...
         sw_cpu_prev <= input_type[5];
   end
   assign cpu_start_sw_pulse = input_type[5] && !sw_cpu_prev;
   assign arm_write_to_imem_sw = input_type[6];
   ...
   // CPU start + ARM IMEM write can be driven from top-level pins or via UDP software regs:
   // input_type[5] pulse => cpu_start, input_type[6] => arm_write_to_imem using address/imem regs.
   wire cpu_start_merged = cpu_start | cpu_start_sw_pulse;
   wire arm_write_to_imem_merged = arm_write_to_imem | arm_write_to_imem_sw;
   wire [8:0]  arm_addr_imem_host_merged = arm_write_to_imem_sw ? address[8:0] : arm_addr_imem_host;
   wire [31:0] arm_data_imem_host_merged = arm_write_to_imem_sw ? imem         : arm_data_imem_host;

   pipeline_pseudoARM arm_cpu (
      .clk                 (clk),
      .rst                 (reset),
      .cpu_start           (cpu_start_merged),
      .mem_stall           (arm_mem_stall),
      .write_to_imem       (arm_write_to_imem_merged),
      .write_to_dmem       (write_to_dmem),
      .addr_imem_host      (arm_addr_imem_host_merged),
      .data_imem_host      (arm_data_imem_host_merged),
   ...
   assign in_rdy   = out_rdy && !net_tx_valid;
