module ifetch (
    ...
    // GPU start: one-shot PC redirect when gpu_redirect pulses (e.g. GPU_START edge)
    input         fetch_active,
    input         gpu_redirect,
    input  [8:0]  gpu_pc_target,
    ...
    // PC: host imem port -> stall -> GPU start redirect -> branch
    // -> hold when not in active fetch window -> increment.
    assign pc_1 = (write_to_imem || read_req_imem) ? pc :
                  stall                          ? pc :
                  gpu_redirect                   ? gpu_pc_target :
                  PCSrc                          ? branch_target :
                  (!fetch_active)                ? pc :
                  pc + 9'd1;
