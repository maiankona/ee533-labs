...
// - FIFO head/tail decode uses low byte FE/FF when |addr[11:8] (same as gpu_fifo_mem host).
...
module pipeline_pseudoARM(
    ...
    // 0xF0–0xFF: gpu_control_interface_2 (F0 cfg base, F1 len, F2 dispatch, F3 irq ack).
    // Do not route these through shared BRAM (cpu_pipe_*); that duplicated STRs and
    // corrupted the same SRAM as GPU DMEM, zeroing / trashing tensor and host readback.
    // Exclude FIFO-mapped cycles: head/tail use low byte FE/FF (same as gpu_fifo_mem host).
    wire        is_gpu_ctrl_mmio = !is_fifo_mapped && (store_addr >= 8'hF0);
    // FIFO MMIO low byte matches gpu_fifo_mem host decode (addr_dmem_host FE/FF), not +0/+4.
    wire        fifo_head      = is_fifo_mapped && (store_addr == 8'hFE);
    wire        fifo_tail      = is_fifo_mapped && (store_addr == 8'hFF);
    wire        fifo_ctrl      = is_fifo_mapped && (store_addr == 8'h08);
    wire        fifo_data      = is_fifo_mapped && (me_alu_res[11:8] == 4'd2);
    wire        true_store     = !is_fifo_mapped && !is_gpu_ctrl_mmio && me_wme;
    ...
    assign cpu_pipe_mem_read  = !is_fifo_mapped && !is_gpu_ctrl_mmio && ex_me_q[5] && !me_wme;
    ...
    assign alu_result_detected = me_wme && !is_fifo_mapped && !is_gpu_ctrl_mmio;
