...
sleep(1);
regwrite($IDS_DMEM_IN_LO, 0x4080bf00);
sleep(1);
regwrite($IDS_DMEM_IN_HI, 0x4040c000);
sleep(1);
regwrite($IDS_INPUT_TYPE, $DMEM_WE);
sleep(1);
regwrite($IDS_INPUT_TYPE, 0x00);
print "  DMEM[0x70] word: lo=0x4080BF00 hi=0x4040C000 (A)\n";

print "\n  Verify DMEM readback:\n";
my $verify_a_lo = dmem_read32(0x70);
my $verify_a_hi = dmem_read32(0x71);
printf("    A[0x70] = 0x%08X %08X \n", 
$verify_a_lo, $verify_a_hi);
if ($verify_a_lo != 0x4080bf00 || $verify_a_hi != 0x4040c000) {
...
my $expected_hi = 0x40400000;
my $expected_lo = 0x40800000;

print "\nSTEP 5: Verify\n";
if ($result_lo == $expected_lo && $result_hi == $expected_hi) {
    print "  ✓ PASS! RELU 40800000 40400000\n";
} else {
    print "  ✗ FAIL!\n";
