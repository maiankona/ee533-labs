...
*/
   ...
   // CLKDV_DIVIDE=2 gives 62.5MHz CLKDV from 125MHz CLKIN.
   // CLKFB must be driven by CLK0 (Virtex-II Pro rule).
   // CLK0 feedback path uses a separate BUFG — not the system clock BUFGMUX.
   DCM #(.CLKDV_DIVIDE(2.0)) CORE_DCM_CLK (
                     .CLKIN(core_clk_ibuf),
                     .CLKFB(core_clk0_buf),  // feedback via CLK0 BUFG (125MHz)
                     .DSSEN(1'b0),
                     .PSINCDEC(1'b0),
                     .PSEN(1'b0),
                     .PSCLK(1'b0),
                     .RST(nf2_reset & ~disable_reset),
                     .CLK0(core_clk0),       // 125MHz — feedback only
                     .CLK90(),
                     .CLK180(),
                     .CLK270(),
                     .CLK2X(),
                     .CLK2X180(),
                     .CLKDV(core_clkdv),    // 62.5MHz system clock
                     .CLKFX(),
                     .CLKFX180(),
                     .PSDONE(),
                     .STATUS(),
   ...
   // CLK0 feedback buffer — required for DCM lock (Virtex-II Pro)
   BUFG BUFG_CORE_FB (.I(core_clk0), .O(core_clk0_buf));

   // Route 62.5MHz CLKDV as core_clk_int
   BUFGMUX BUFGMUX_CORE_CLK (
                             .O(core_clk_int),
                             .I1(cpci_clk_ibuf),  // not used.
                             .I0(core_clkdv),     // 62.5MHz
                             .S(1'b0)
                             );
